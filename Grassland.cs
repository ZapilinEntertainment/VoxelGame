...
public class Grassland
{
    ...
    static List<float> plantCreateTimer = new List<float>();
    ...
    public static int GrasslandUpdate(float tax)
    {
        ...
        else
        {
            ...
            {
                if (grasslandList[i] == null)
                {
                    grasslandList.RemoveAt(i);
                    plantCreateTimer.RemoveAt(i);
                    continue;
                }
                i++;
            }
        ...
        if (tax != 0)
        {
            ...
            else
            {
                tax *= -1;
                int i = 0;
                int tax2 = Mathf.RoundToInt(tax / (float)grasslandList.Count);
                while (i < grasslandList.Count)
                {
                    i++;
                ...
                if (stage > 2)
                {
                    ...
                    while (lifepower > 2 * LIFEPOWER_TO_PREPARE & i < plants.Count)
                    {
                        ...
                        if (p.lifepower < p.lifepowerToGrow)
                        {
                            ...
                            if (donation > p.maxLifeTransfer) donation = p.maxLifeTransfer;
                    ...
                    if (plantCreateTimer[a] <= 0)
                    {
                        ...
                        if (lifepower > (2 * LIFEPOWER_TO_PREPARE + cost) & myBlock.cellsStatus != 1)
                        {
                            ...
                            plantCreateTimer[a] = LIFE_CREATION_TIMER;
                        }
                    }
                }
            }
    ...
    public static Grassland Create(SurfaceBlock sblock)
    {
        ...
        plantCreateTimer.Add(LIFE_CREATION_TIMER * Random.value);
        return gl;
    }

    byte CheckGrasslandStage()
    {
