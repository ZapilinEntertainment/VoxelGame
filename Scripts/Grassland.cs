...
public sealed class Grassland
{
            ...
            {                
                if (stage > 2 & plants.Count < MAX_PLANTS_COUNT )
                {
                    ...
                    if (gl.plantCreateCooldown <= 0)
                    {
                        ...
                        if (lifepower > (2 * LIFEPOWER_TO_PREPARE + cost) & myBlock.noEmptySpace != true)
                        {
    ...
    public void AddLifepowerAndCalculate(int count)
    {
        ...
        if (myBlock.noEmptySpace != false)
        {
            foreach (Structure s in myBlock.structures)
            {
        ...
        if (lifepower > 2 * LIFEPOWER_TO_PREPARE)
        {
            ...
            if (treesCount != 0)
            {
                while (i < treesCount & freeEnergy > 0 & myBlock.noEmptySpace != true)
                {
            ...
            if (existingPlantsCount != 0 & freeEnergy >= lifepowerDosis)
            {
                i = 0;
                Plant p = null;
                for (; i < myBlock.structures.Count; i++)
                {
                    p = myBlock.structures[i] as Plant;
                    if (p != null)
                    {
