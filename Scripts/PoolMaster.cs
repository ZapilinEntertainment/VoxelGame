...
public enum MaterialPack : byte { Default, Advanced, Mono }
...
public sealed class PoolMaster : MonoBehaviour
{
    ...
    public static MaterialPack materialPack { get; private set; }
    ...
    public const int NO_MATERIAL_ID = -1, MATERIAL_ADVANCED_COVERING_ID = -2, MATERIAL_GRASS_100_ID = -3, MATERIAL_GRASS_80_ID = -4, MATERIAL_GRASS_60_ID = -5,
        ...
        MATERIAL_WHITEWALL_ID = -11, MATERIAL_ENERGY_MATERIAL = -12, MATERIAL_GLASS_ACTIVE = -13, MATERIAL_GLASS_DISABLED = -14;
    ...
    public void Load()
    {
        ...
        {
            materialPack = MaterialPack.Advanced;
        }
        else materialPack = MaterialPack.Default;

        shadowCasting = !(materialPack == MaterialPack.Default);
        ...
        switch (materialPack)
        {
            case MaterialPack.Mono:
                {
                    basic_material = Resources.Load<Material>("Materials/AllInOne");
                    glassMaterial = basic_material;
                    metal_material = basic_material;
                    green_material = basic_material;
                    glassMaterial_disabled = basic_material;
                }
                break;
            case MaterialPack.Advanced:
                {
        ...
        if (rrs != null && rrs.Length != 0) ReplaceMaterials(rrs, materialPack);
    ...
    public static void ChangeQualityLevel(int newLevel)
    {
        if (qualityLevel != newLevel)
        {
            ...
            var rrs = Component.FindObjectsOfType<Renderer>();
            ReplaceMaterials(rrs, materialPack);
        }
    }
            ...
            case ShipType.Passenger:
                {
                    ...
                    if (!found)
                    {
                        ...
                        if (materialPack != MaterialPack.Default) ReplaceMaterials(s.gameObject, materialPack);
                    }
                    break;
                }
            case ShipType.Cargo:
                {
                    ...
                    if (!found)
                    {
                        switch (level)
                        {
                            ...
                            default:
                                s = Instantiate(Resources.Load<GameObject>("Prefs/lightCargoShip")).GetComponent<Ship>();
                                break;
                        }
                        if (materialPack != MaterialPack.Default) ReplaceMaterials(s.gameObject, materialPack);
                    }
    ...
    public static GameObject GetRooftop(bool peak, bool artificial, byte number)
    {
        ...
        if (materialPack != MaterialPack.Default) ReplaceMaterials(g, materialPack);
        return g;
    }
    ...
    {        
        ...
        float p = 0.25f, add;
        if (materialPack == MaterialPack.Default || materialPack == MaterialPack.Advanced)
        {
            p = 0.25f;
            add = ((Random.value > 0.5) ? p : 0);
            Vector2[] borders;
            switch (materialID)
            {
                case ResourceType.STONE_ID:
                    borders = new Vector2[] { new Vector2(3 * p, 2 * p), new Vector2(3 * p, 3 * p), new Vector2(4 * p, 3 * p), new Vector2(4 * p, 2 * p) };
                    break;
                case ResourceType.DIRT_ID:
                    borders = new Vector2[] { new Vector2(p, 2 * p), new Vector2(p, 3 * p), new Vector2(2 * p, 3 * p), new Vector2(2 * p, 2 * p) };
                    break;
                case ResourceType.LUMBER_ID:
                    borders = new Vector2[] { new Vector2(0, 2 * p), new Vector2(0, 3 * p), new Vector2(p, 3 * p), new Vector2(p, 2 * p) };
                    break;
                case ResourceType.METAL_K_ORE_ID:
                case ResourceType.METAL_K_ID:
                    borders = new Vector2[] { new Vector2(0, 3 * p), new Vector2(0, 4 * p), new Vector2(p, 4 * p), new Vector2(p, 3 * p) };
                    break;
                case ResourceType.METAL_M_ORE_ID:
                case ResourceType.METAL_M_ID:
                    borders = new Vector2[] { new Vector2(p, 3 * p), new Vector2(p, 4 * p), new Vector2(2 * p, 4 * p), new Vector2(2 * p, 3 * p) };
                    break;
                case ResourceType.METAL_E_ORE_ID:
                case ResourceType.METAL_E_ID:
                    borders = new Vector2[] { new Vector2(2 * p, 3 * p), new Vector2(2 * p, 4 * p), new Vector2(3 * p, 4 * p), new Vector2(3 * p, 3 * p) };
                    break;
                case ResourceType.METAL_N_ORE_ID:
                case ResourceType.METAL_N_ID:
                    borders = new Vector2[] { new Vector2(3 * p, 3 * p), new Vector2(3 * p, 4 * p), new Vector2(4 * p, 4 * p), new Vector2(4 * p, 3 * p) };
                    break;
                ...
                case ResourceType.MINERAL_L_ID:
                    borders = new Vector2[] { new Vector2(0, p), new Vector2(0, 2 * p), new Vector2(p, 2 * p), new Vector2(p, p) };
                    break;
                ...
                case ResourceType.SNOW_ID:
                    borders = new Vector2[] { new Vector2(p, p), new Vector2(p, 2 * p), new Vector2(2 * p, 2 * p), new Vector2(2 * p, p) };
                    break;
                case ResourceType.FERTILE_SOIL_ID:
                    borders = new Vector2[] { new Vector2(2 * p, 2 * p), new Vector2(2 * p, 3 * p), new Vector2(3 * p, 3 * p), new Vector2(3 * p, 2 * p) };
                    break;
                case MATERIAL_ADVANCED_COVERING_ID:
                    borders = new Vector2[] { new Vector2(3 * p, p), new Vector2(3 * p, 2 * p), new Vector2(4 * p, 2 * p), new Vector2(4 * p, p) };
                    break;
                case MATERIAL_GRASS_100_ID:
                    borders = new Vector2[] { new Vector2(p, 0), new Vector2(p, p), new Vector2(2 * p, p), new Vector2(2 * p, 0) };
                    break;
                case MATERIAL_GRASS_80_ID:
                    borders = new Vector2[] { new Vector2(2 * p + add, 0), new Vector2(2 * p + add, p), new Vector2(3 * p + add, p), new Vector2(3 * p + add, 0) };
                    break;
                case MATERIAL_GRASS_60_ID:
                    borders = new Vector2[] { new Vector2(2 * p + add, p), new Vector2(2 * p + add, 2 * p), new Vector2(3 * p + add, 2 * p), new Vector2(3 * p + add, p) };
                    break;
                case MATERIAL_GRASS_40_ID:
                    borders = new Vector2[] { new Vector2(2 * p + add, 2 * p), new Vector2(2 * p + add, 3 * p), new Vector2(3 * p + add, 3 * p), new Vector2(3 * p + add, 2 * p) };
                    break;
                case MATERIAL_GRASS_20_ID:
                    borders = new Vector2[] { new Vector2(2 * p + add, 3 * p), new Vector2(2 * p + add, 4 * p), new Vector2(3 * p + add, 4 * p), new Vector2(3 * p + add, 3 * p) };
                    break;
                case MATERIAL_LEAVES_ID:
                    borders = new Vector2[] { Vector2.zero, Vector2.up * p, Vector2.one * p, Vector2.right * p };
                    break;
                ...
                case MATERIAL_WHITEWALL_ID:
                    borders = new Vector2[] { new Vector2(2 * p, p), new Vector2(2 * p, 2 * p), new Vector2(3 * p, 2 * p), new Vector2(3 * p, p) };
                    break;
        ...
        {
            p = 0.125f;
            add = ((Random.value > 0.5) ? p : 0f);
            Vector2 pt;
            switch (materialID)
            {
                case ResourceType.STONE_ID:
                    pt = new Vector2(3.5f * p, 6.5f * p);
                    break;
                case ResourceType.DIRT_ID:
                    pt = new Vector2(1.5f * p, 6.5f * p);
                    break;
                case ResourceType.LUMBER_ID:
                    pt = new Vector2(0.5f * p, 6.5f * p);
                    break;
                case ResourceType.METAL_K_ORE_ID:
                case ResourceType.METAL_K_ID:
                    pt = new Vector2(4.5f * p, 7.5f *p);
                    break;
                case ResourceType.METAL_M_ORE_ID:
                case ResourceType.METAL_M_ID:
                    pt = new Vector2(5.5f * p, 7.5f * p);
                    break;
                case ResourceType.METAL_E_ORE_ID:
                case ResourceType.METAL_E_ID:
                    pt = new Vector2(6.5f *p, 7.5f * p);
                    break;
                case ResourceType.METAL_N_ORE_ID:
                case ResourceType.METAL_N_ID:
                    pt = new Vector2(7.5f * p, 7.5f * p);
                    break;
                case ResourceType.METAL_P_ORE_ID:
                case ResourceType.METAL_P_ID:
                    pt = new Vector2(4.5f * p, 6.5f * p);
                    break;
                case ResourceType.METAL_S_ORE_ID:
                case ResourceType.METAL_S_ID:
                    pt = new Vector2(5.5f * p, 6.5f * p);
                    break;
                case ResourceType.MINERAL_F_ID:
                    pt = new Vector2(3.5f * p, 7.5f * p);
                    break;
                case ResourceType.MINERAL_L_ID:
                    pt = new Vector2(0.5f * p, 5.5f * p);
                    break;
                case ResourceType.PLASTICS_ID:
                    pt = new Vector2(1.5f * p, 7.5f * p);
                    break;
                case ResourceType.CONCRETE_ID:
                    pt = new Vector2(0.5f * p, 7.5f * p);
                    break;
                case ResourceType.SNOW_ID:
                    pt = new Vector2(1.5f * p, 5.5f * p);
                    break;
                case ResourceType.FERTILE_SOIL_ID:
                    pt = new Vector2(2.5f * p, 6.5f * p);
                    break;
                case MATERIAL_ADVANCED_COVERING_ID:
                    pt = new Vector2(3.5f * p, 6.5f * p);
                    break;
                case MATERIAL_GRASS_100_ID:
                    pt = new Vector2(1.5f * p, 0.5f * p);
                    break;
                case MATERIAL_GRASS_80_ID:
                    pt = new Vector2(2.5f * p + add, 0.5f * p);
                    break;
                case MATERIAL_GRASS_60_ID:
                    pt = new Vector2(2.5f * p + add, 1.5f * p);
                    break;
                case MATERIAL_GRASS_40_ID:
                    pt = new Vector2(2.5f * p + add, 2.5f * p);
                    break;
                case MATERIAL_GRASS_20_ID:
                    pt = new Vector2(2.5f * p + add, 3.5f * p);
                    break;
                case MATERIAL_LEAVES_ID:
                    pt = new Vector2(0.5f + add, 0.5f * p);
                    break;
                case MATERIAL_WHITE_METAL_ID:
                    pt = new Vector2(6.5f * p, 6.5f * p);
                    break;
                case MATERIAL_DEAD_LUMBER_ID:
                    pt = new Vector2(2.5f * p, 7.5f * p);
                    break;
                case MATERIAL_WHITEWALL_ID:
                    pt = new Vector2(2.5f * p, 5.5f * p);
                    break;
                case ResourceType.GRAPHONIUM_ID:
                    pt = new Vector2(5.5f * p, 3.5f * p);
                    break;
                case MATERIAL_GLASS_ACTIVE:
                    pt = new Vector2(4.5f * p, 3.5f * p);
                    break;
                case MATERIAL_GLASS_DISABLED:
                    pt = new Vector2(4.5f * p, 2.5f * p);
                    break;
                default: pt = new Vector2(4.5f * p, 4.5f * p); break;
            }
            for (int i = 0; i< uvEdited.Length; i++)
            {
                uvEdited[i] = pt;
    ...
    {
        if (materialPack == MaterialPack.Mono) return MaterialType.Basic;
        else
        {
            switch (materialID)
            {
    ...
    {
        if (materialPack == MaterialPack.Mono) return basic_material;
        else
        {
            switch (mtype)
            {
    ...
    {
        ReplaceMaterials(g, MaterialPack.Default);
    }
    public static void ReplaceMaterials(Renderer[] rrs)
    {
        ReplaceMaterials(rrs, MaterialPack.Default);
    }
    public static void ReplaceMaterials(GameObject g, MaterialPack mpack)
    {
    ...
    public static void ReplaceMaterials(Renderer[] rrs, MaterialPack mpack)
    {
        Material[] materials;
        if (mpack == materialPack)
        {
        ...
        else
        {
            switch (mpack)
            {
                ...
                case MaterialPack.Mono:
                    {
                        var m = Resources.Load<Material>("Materials/AllInOne");
                        materials = new Material[5]
                        {
                            m,m,m,m,m
                        };
        ...
        {
            bool castShadows = false, receiveShadows = false, monopack = materialPack == MaterialPack.Mono;
            if (mr.sharedMaterial != null)
            {
                switch (mr.sharedMaterial.name)
                {
                    ...
                    case "ShadedBillboard":
                        if (materialPack != MaterialPack.Default) mr.sharedMaterial = billboardShadedMaterial;
