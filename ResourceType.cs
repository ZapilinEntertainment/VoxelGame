...
public class ResourceType {
	...
	public ResourceType( int f_id, float f_mass, float f_toughness, Material f_material) {
		ID = f_id;
		mass = f_mass;
		...
		Nothing = new ResourceType(0, 0, 0, Resources.Load<Material>("Materials/default")); 

		Food = new ResourceType(FOOD_ID, 0.1f, 0.1f, Resources.Load<Material>("Materials/default"));
		prices[FOOD_ID] = 1; demand[FOOD_ID] = 2;

		metal_K = new ResourceType( METAL_K_ID, 0.7f, 50, Resources.Load<Material>("Materials/MetalK") );
		prices[METAL_K_ID] = 3; demand[METAL_K_ID] = 5;
		metal_M = new ResourceType( METAL_M_ID, 0.6f, 35,  Resources.Load<Material>("Materials/MetalM"));
		prices[METAL_M_ID] = 5; demand[METAL_M_ID] = 5;
		metal_E = new ResourceType( METAL_E_ID, 0.3f, 3,  Resources.Load<Material>("Materials/MetalE"));
		prices[METAL_E_ID] = 7; demand[METAL_E_ID] = 5;
		metal_N = new ResourceType( METAL_N_ID, 2, 3,  Resources.Load<Material>("Materials/MetalN"));
		prices[METAL_N_ID] = 25; demand[METAL_N_ID] = 10;
		metal_P = new ResourceType( METAL_P_ID, 0.63f, 32,  Resources.Load<Material>("Materials/MetalP"));
		prices[METAL_P_ID] = 5; demand[METAL_P_ID] = 1;
		metal_S= new ResourceType( METAL_S_ID, 0.2f, 40,  Resources.Load<Material>("Materials/MetalS"));
		prices[METAL_S_ID] = 15; demand[METAL_S_ID] = 4;

		float lowK = 0.7f, ore_lowK = 0.5f;
		metal_K_ore = new ResourceType( METAL_K_ORE_ID, 2.7f, 25,  Resources.Load<Material>("Materials/MetalK"));
		prices[METAL_K_ORE_ID] = prices[METAL_K_ID] * lowK; demand[METAL_K_ORE_ID] = demand[METAL_K_ID] * ore_lowK;
		metal_M_ore = new ResourceType( METAL_M_ORE_ID, 2.6f, 17.5f, Resources.Load<Material>("Materials/MetalM"));
		prices[METAL_M_ORE_ID] = prices[METAL_M_ID] * lowK; demand[METAL_M_ORE_ID] = demand[METAL_M_ID] * ore_lowK;
		metal_E_ore = new ResourceType( METAL_E_ORE_ID, 2.3f, 1.5f, Resources.Load<Material>("Materials/MetalE"));
		prices[METAL_E_ORE_ID] = prices[METAL_E_ID] * lowK; demand[METAL_E_ORE_ID] = demand[METAL_E_ID] * ore_lowK;
		metal_N_ore = new ResourceType( METAL_N_ORE_ID, 4, 1.5f, Resources.Load<Material>("Materials/MetalN"));
		prices[METAL_N_ORE_ID] = prices[METAL_N_ID] * lowK; demand[METAL_N_ORE_ID] = demand[METAL_N_ID] * ore_lowK;
		metal_P_ore = new ResourceType( METAL_P_ORE_ID, 2.63f, 16, Resources.Load<Material>("Materials/MetalP"));
		prices[METAL_P_ORE_ID] = prices[METAL_P_ID] * lowK; demand[METAL_P_ORE_ID] = demand[METAL_P_ID] * ore_lowK;
		metal_S_ore = new ResourceType( METAL_S_ORE_ID, 2.2f, 20, Resources.Load<Material>("Materials/MetalS"));
		prices[METAL_S_ORE_ID] = prices[METAL_S_ID] * lowK; demand[METAL_S_ORE_ID] = demand[METAL_S_ID] * ore_lowK;

		mineral_F = new ResourceType( MINERAL_F_ID,1.1f, 1, Resources.Load<Material>("Materials/MineralF"));
		prices[MINERAL_F_ID] = 3; demand[MINERAL_F_ID] = 10;
		Fuel = new ResourceType( FUEL_ID, 0.1f, 0, PoolMaster.default_material);
		prices[FUEL_ID] = prices[MINERAL_F_ID] * 4; demand[FUEL_ID] = 45;

		Plastics = new ResourceType( PLASTICS_ID ,0.5f, 10, Resources.Load<Material>("Materials/Plastic"));
		prices[PLASTICS_ID] = 1.5f; demand[PLASTICS_ID] = 2;
		Lumber = new ResourceType( LUMBER_ID, 0.5f, 5,  Resources.Load<Material>("Materials/Lumber")); 
		prices[LUMBER_ID] = prices[PLASTICS_ID] / 10f; demand[LUMBER_ID] = demand[PLASTICS_ID] / 6f;
		mineral_L = new ResourceType(  MINERAL_L_ID ,1, 2, Resources.Load<Material>("Materials/MineralL"));
		prices[MINERAL_L_ID] = prices[PLASTICS_ID] / 2f; demand [MINERAL_L_ID] = demand[PLASTICS_ID] *ore_lowK;

		Stone = new ResourceType( STONE_ID, 2.5f, 30,  Resources.Load<Material>("Materials/Stone"));
		prices[STONE_ID] = 0.2f; demand[STONE_ID] = 0.5f;
		Concrete = new ResourceType( CONCRETE_ID, 3, 38, Resources.Load<Material>("Materials/Concrete"));
		prices[CONCRETE_ID] = prices[STONE_ID] * 1.5f; demand[CONCRETE_ID] = demand[STONE_ID] * 1.8f;

		Dirt = new ResourceType (DIRT_ID, 1, 1,   Resources.Load<Material>("Materials/Dirt"));
		...
		FertileSoil = new ResourceType( FERTILE_SOIL_ID, 1, 1, fertileSoil_material);
		prices[FERTILE_SOIL_ID] = prices[DIRT_ID]; demand[FERTILE_SOIL_ID] = demand[DIRT_ID];

		Graphonium = new ResourceType( GRAPHONIUM_ID, 4, 2.5f, PoolMaster.energy_material);
		prices[GRAPHONIUM_ID] = prices[METAL_N_ID * 3];
		demand[GRAPHONIUM_ID] = 1;

		Supplies = new ResourceType( SUPPLIES_ID, 0.1f, 0.1f, PoolMaster.default_material);
